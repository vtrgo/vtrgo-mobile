...
const App = () => {
  ...
  const scanFloatTab = async () => {
    try {
      ...
      let floatTab = null;
      try {
        floatTab = JSON.parse(jsonString);
        if (!Array.isArray(floatTab)) throw new Error('Expected JSON array');
        console.log('✅ Parsed Float Table:', JSON.stringify(floatTab, null, 2));

        // ✅ Update state so UI re-renders
        setFloatData(floatTab);

        const formattedData = floatTab.map(d => ({
          ...d,
          timestamp: new Date(d.time).getTime(), // or just `new Date(d.time)`
        }));
        console.log('timestamps', JSON.stringify(formattedData, null, 2));

        setFormattedFloatData(formattedData);
      } catch (parseError) {
        console.error('❌ Failed to parse float tab:', parseError.message);
        console.log('📝 Raw string:', jsonString);
        return;
      }
  ...
  const writeNfcFloatRequest = async (fieldName) => {
    const payload = {
      cmd: "float_range",
      field: fieldName,
      start: "-60m",
    ...
    try {
      ...
      await NfcManager.ndefHandler.writeNdefMessage(bytes); // ✅ correct method
                    ...
                    <View key={idx} style={styles.itemRow}>
                      ...
                      {sectionTitle === 'float_averages' && (
                        <TouchableOpacity
                          ...
                          onPress={() => writeNfcFloatRequest(key)}
                        >
